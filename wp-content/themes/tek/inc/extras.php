	...
	function stm_breadcrumbs( $showCurrent = true ) {
		...
		if ( is_home() || is_front_page() ) {
			...
			if ( is_category() ) {
				...
				echo balanceTags( $before . sprintf( __( 'Archive by category "%s"', STM_DOMAIN ), single_cat_title( '', false ) ) . $after, true );
			} elseif ( is_search() ) {
				echo balanceTags( $before . sprintf( __( 'Search results for "%s"', STM_DOMAIN ), get_search_query() ) . $after, true );
			} elseif ( is_day() ) {
				...
				echo balanceTags( $before . sprintf( __( 'Posts tagged "%s"', STM_DOMAIN ), single_tag_title( '', false ) ) . $after, true );

			} elseif ( is_author() ) {
				global $author;
				$userdata = get_userdata( $author );
				echo balanceTags( $before . sprintf( __( 'Articles posted by %s', STM_DOMAIN ), $userdata->display_name ) . $after, true );

			} elseif ( is_404() ) {
				echo balanceTags( $before . __( 'Error 404', STM_DOMAIN ) . $after, true );
			}
			...
			if ( get_query_var( 'paged' ) ) {
				...
				echo __( 'Page', STM_DOMAIN ) . ' ' . get_query_var( 'paged' );
				if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) {
					echo ')';
				}
			}
	...
	function stm_comment($comment, $args, $depth) {
		...
		<div class="comment-info clearfix">
			...
			<div class="comment-meta commentmetadata">
				...
				<?php comment_reply_link( array_merge( $args, array( 'reply_text' => __( '<i class="fa fa-reply"></i> Reply', STM_DOMAIN ), 'add_below' => $add_below, 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) ); ?>
				<?php edit_comment_link( __( 'Edit', STM_DOMAIN ), '  ', '' ); ?>
			</div>
		...
		$fields    = array(
			...
			'url'    => '<div class="col-xs-12 col-sm-4 col-md-4 col-lg-4">
						<div class="input-group comment-form-url">
							<input placeholder="' . __( 'Website', STM_DOMAIN ) . '" class="form-control" name="url" type="text" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="30" />
						</div>
					</div></div>'
		);
...
function add_metaboxes(){
	STM_PostType::addMetaBox( 'page_options', __( 'Page Options', STM_POST_TYPE ), array( 'page', 'post', 'service', 'project', 'product', 'vacancy' ), '', '', '', array(
		'fields' => array(
			'separator_page_header' => array(
				'label'   => __( 'Page Header', STM_POST_TYPE ),
				'type'    => 'separator'
			),
	...
	STM_PostType::addMetaBox( 'vacancy_info', __( '', STM_POST_TYPE ), array( 'vacancy' ), '', '', '', array(
		'fields' => array(
			...
			'vacancy_department'   => array(
				'label' => __( 'Department', STM_POST_TYPE ),
				'type'  => 'text'
			),
			'vacancy_job_type'   => array(
				'label' => __( 'Job Type', STM_POST_TYPE ),
				'type'  => 'text'
			),
			'vacancy_education'   => array(
				'label' => __( 'Education', STM_POST_TYPE ),
				'type'  => 'text'
			),
			'vacancy_compensation'   => array(
				'label' => __( 'Compensation', STM_POST_TYPE ),
				'type'  => 'text'
			)
		)
	) );
